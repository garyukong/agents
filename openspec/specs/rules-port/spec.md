...
The `rules port` command converts universal rule files into provider-specific syntax for claude-code, windsurf, copilot-vscode, and copilot-jetbrains. It reads a canonical frontmatter schema and outputs the appropriate format for each target provider.

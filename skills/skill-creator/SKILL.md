...
## Local Defaults (override standard behaviour)
...
```bash
cd /Users/garykong/PycharmProjects/agents/skills
git add <skill-name>/
git commit -m "feat(skills): add <skill-name> skill"
git push
```

Replace `<skill-name>` with the actual skill directory name. Use conventional commit format (`feat(skills): ...`). Describe what the skill does in the commit message, not just its name (e.g. `feat(skills): add pdf-extract skill for structured data extraction from PDFs`). Ask for confirmation before pushing if you're unsure the skill is ready.

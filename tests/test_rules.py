...
from rules import Provider, RulesManager, TriggerMode
...
class TestConvertClaudeCode:
    def test_glob_trigger_produces_globs_list(self, tmp_path):
        ...
        # Then each pattern appears as a YAML list item under globs:
        assert "globs:" in result
        assert "  - **/*.py" in result
        assert "  - pyproject.toml" in result

    def test_model_decision_omits_globs(self, tmp_path):
        # Given a rule with trigger: model_decision (unsupported mode)
        mgr = _manager(tmp_path)
        fm = {"trigger": "model_decision", "patterns": ["**/*.py"]}
        # When converted to claude-code
        result = mgr._convert(Provider.CLAUDE_CODE, fm, "body", "universal/rule.md")
        # Then no globs field is emitted (always-on)
        assert "globs" not in result

    def test_manual_omits_globs(self, tmp_path):
    ...
    def test_description_included(self, tmp_path):
        # Given a rule with a description field
        mgr = _manager(tmp_path)
        fm = {"description": "My desc"}
        # When converted to claude-code
        result = mgr._convert(Provider.CLAUDE_CODE, fm, "body", "universal/rule.md")
        # Then the description is present in the frontmatter block
...
class TestConvertWindsurf:
    def test_glob_trigger_comma_separated(self, tmp_path):
        # Given a rule with trigger: glob and multiple patterns
        mgr = _manager(tmp_path)
        fm = {"trigger": "glob", "patterns": ["**/*.py", "**/conftest.py"]}
        # When converted to windsurf
        result = mgr._convert(Provider.WINDSURF, fm, "body", "universal/rule.md")
        # Then trigger and globs appear with comma-separated patterns
        assert "trigger: glob" in result
        assert "globs: **/*.py, **/conftest.py" in result

    def test_always_on_omits_globs(self, tmp_path):
        # Given a rule with no trigger (always-on)
        mgr = _manager(tmp_path)
        # When converted to windsurf
        result = mgr._convert(Provider.WINDSURF, {}, "body", "universal/rule.md")
        # Then neither globs nor trigger fields are emitted
        assert "globs" not in result
...
class TestPortRule:
    ...
    def test_writes_output_file(self, tmp_path):
        ...
        # Then the output file is written with globs
        out = tmp_path / "rules" / "claude-code" / "my-rule.md"
        assert out.exists()
        assert "globs:" in out.read_text()
